defmodule OfferServiceWeb.OfferControllerTest do
  ...
  describe "POST /api/v1/requests/:request_id/offers/:offer_id/accept" do
    test "200 returns ONLY the generic transition outcome (JEB-1474)", %{conn: conn} do
      ...
      # The shared accept endpoint returns the accepted offer id and the
      # rejected sibling ids — nothing product-specific. OTP, chat-thread
      # linkage and notification fan-out are owned by the consuming gateway.
    ...
    test "200 replay returns identical body & x-idempotency-replay=true (JEB-49 / AC2)", %{
      ...
      # Replay: same client_id, same key, same payload → saga not re-run,
      # identical body.
      second =
        Phoenix.ConnTest.build_conn()
        |> put_req_header("x-user-id", request.client_id)
